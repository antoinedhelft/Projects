...
def render():
    # st.set_page_config(page_title="1 - Introduction", layout="wide") # Removed
    ...
    with tabs[2]:
        ...
        with col_c:
            # Check if image exists, otherwise skip or show warning
            if (img_path / "CryptoBot Architecture.png").exists():
                st.image(str(img_path / "CryptoBot Architecture.png"), width=1000)
            else:
                st.warning(f"Image not found: {img_path / 'CryptoBot Architecture.png'}")
